...
void mergeKipad(hls::stream<ap_uint<blockSize * 8> >& kipadStrm,
    ...
    while (!eLenStrm2.read()) {
        ...
        // kipad: 一拍两词（dataW=32，512bit/32=16，偶数）
        int total_words = (blockSize * 8) / dataW;
        for (int i = 0; i < total_words; i += 2) {
#pragma HLS pipeline II = 1
            ap_uint<dataW> w0 = kipad.range(blockSize * 8 - 1, blockSize * 8 - dataW);
            kipad <<= dataW;
            ap_uint<dataW> w1 = kipad.range(blockSize * 8 - 1, blockSize * 8 - dataW);
            kipad <<= dataW;
            mergeKipadStrm.write(w0);
            mergeKipadStrm.write(w1);
        }

        kopadOutStrm.write(kopadInStrm.read());

        // msg: 保持一拍一词（消息长度不固定，避免溢出复杂度）
...
void mergeKopad(hls::stream<ap_uint<blockSize * 8> >& kopadStrm,
    ...
    while (!eMsgHashStrm.read()) {
        ...
        for (int i = 0; i < total_words; i += 2) {
#pragma HLS pipeline II = 1
            ap_uint<dataW> w0 = kopad.range(blockSize * 8 - 1, blockSize * 8 - dataW);
            kopad <<= dataW;
            ap_uint<dataW> w1 = kopad.range(blockSize * 8 - 1, blockSize * 8 - dataW);
            kopad <<= dataW;
            mergeKopadStrm.write(w0);
            mergeKopadStrm.write(w1);
        }

        // msgHash(256b): 两词/拍（dataW=32 → 8词，偶数）
        int h_words = (hshW + dataW - 1) / dataW;
        for (int i = 0; i < h_words; i += 2) {
#pragma HLS pipeline II = 1
            ap_uint<dataW> w0 = msgHash.range(dataW - 1, 0);
            msgHash >>= dataW;
            ap_uint<dataW> w1 = msgHash.range(dataW - 1, 0);
            msgHash >>= dataW;
            mergeKopadStrm.write(w0);
            mergeKopadStrm.write(w1);
        }
